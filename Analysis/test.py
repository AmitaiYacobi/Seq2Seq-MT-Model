...
def predict_output_symbols(decoder, index_to_symbol_trg, first_symbol, encoder_states, trg_input):
    ...
    for i in range(max_trg_seq_len):
        decoder_output, decoder_hidden_state = decoder(decoder_next_input, decoder_hidden_state, encoder_states)
...
def test(src_input, trg_input, encoder, decoder, index_to_symbol_trg, test_trg_data):
    ...
    with torch.no_grad():
        for i ,(src_seq, trg_seq) in enumerate(zip(src_input, trg_input)):
            ...
            encoder_states = torch.zeros(src_seq_len, encoder.hidden_dim)
            ...
            output_symbols = predict_output_symbols(decoder, index_to_symbol_trg, trg_seq[0], encoder_states, trg_input)
...
def main():
    ...
    decoder = Decoder(vocab_size=len(trg_vocab), embedding_dim=128, encoder_hidden_dim=256, hidden_dim=256)
