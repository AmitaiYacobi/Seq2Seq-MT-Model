...
class Seq2SeqModel:
    ...
    def epoch(self, src, trg):
        ...
        for i , (src_seq, trg_seq) in enumerate(zip(src, trg)):
            ...
            for i in range(trg_seq_len - 1):
                decoder_output, decoder_hidden_state = self.decoder(trg_seq[i], decoder_hidden_state, encoder_states)
    ...
    def predict_output_symbols(self, first_symbol, encoder_states):
        ...
        for i in range(max_trg_seq_len):
            decoder_output, decoder_hidden_state = self.decoder(decoder_next_input, decoder_hidden_state, encoder_states)
